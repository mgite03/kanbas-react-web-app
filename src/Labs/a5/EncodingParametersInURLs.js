...
function EncodingParametersInURLs() {
    ...
    const fetchSum = async (a, b) => {
        const response = await axios.get(`${URL_BASE}/a5/add/${a}/${b}`);
        setResult(response.data)
    }
    const fetchSubtraction = async (a, b) => {
        const response = await axios.get(`${URL_BASE}/a5/subtract/${a}/${b}`);
        setResult(response.data);
    }
    const [welcome, setWelcome] = useState("");
    const fetchWelcome = async () =>  {
        const response = await axios.get(`${URL_BASE}/a5/welcome`);
    ...
    return (
        <div>
            ...
            <a href={`${URL_BASE}/a5/add/${a}/${b}`}
            className="btn btn-primary">
            Add {a} + {b}
            </a>

            <a href={`${URL_BASE}/a5/subtract/${a}/${b}`}
            className="btn btn-primary">
                Subtract {a} - {b}
            </a>
            <h3>Query Parameters</h3>
            <a href={`${URL_BASE}/a5/calculator?operation=add&a=${a}&b=${b}`}
            className="btn btn-primary">
                Add {a} + {b}
            </a>
            <a href={`${URL_BASE}/a5/calculator?operation=subtract&a=${a}&b=${b}`}
            className="btn btn-primary">
                Subtract {a} + {b}
            </a>
